...
int main(int argc, char *argv[]) {
    ...
    while (read.newLine()) {
        ...
        if (line[0] == '>') {
            ...
            newseq += line; //WARNING: THIS ONLY ACCOMODATES A 100 NUCLEOTIDE LINE!
        }
    }
    ...
    for (int j=0; j<inputSequences.size(); j++) {
        ...
        Genome.findPAMs(chromosomeSequence, true, j, pam, true, anti);
        if (offexist) {
            Genome.findPAMs(chromosomeSequence, true, j, Opams[0], false, anti);
        }
        ...
        Genome.findPAMs(reverseSequence, false, j, pam, true, anti);
        if (offexist) {
            Genome.findPAMs(reverseSequence, false, j, Opams[0], false, anti);
        }
    ...
    WriteFile Output;
    Output.setFileName(returnPath + output_file + ".txt");
    Output.retrieveData(&Genome);
    //Output.printInfo(Genome);
    cout << "There were " << Genome.totSize() << " unique sequences." << endl;
    cout << "There were " << Genome.repSize() << " identical sequences." << endl;
    cout << "Finished Creating File.\n To search restart CASPER and select Organism." << endl;
    //Genome->printSequences(); //push this up so you can delete after creating the reverseSequence
    return 0;
}
