...
unsigned long gRNA::insertSequence(long index, int chr, int pamsize, bool anti, bool strand, std::string seq) {
    PAMlocation = index;
    if (!strand) {
        PAMlocation *= -1;
    }
...
std::pair<unsigned long, std::string> gRNA::getVectorPair(unsigned long seed) {
    ...
    std::string totseq = seq2 + seq; //this is backward I think
...
std::string gRNA::baseConvert(unsigned long long number, int base) {
    std::string base64set = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/"; //base64
